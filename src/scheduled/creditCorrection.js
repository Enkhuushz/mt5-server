...
const creditZeroCorrectionWhenBalanceGreatherThanZero = async (group, type) => {
  logger.info(
    `creditZeroCorrectionWhenBalanceGreatherThanZero Started... group = ${group} type = ${type}`
  );
  ...
  logger.info(
    `creditZeroCorrectionWhenBalanceGreatherThanZero End... group = ${group} type = ${type}`
  );
  return "true";
};
